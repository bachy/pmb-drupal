...
// +-------------------------------------------------+
// $Id: pmb_external_search_results.tpl.php,v 1.5 2011-10-21 20:32:24 gueluneau Exp $

$template.= 'External search: '.check_plain($search_terms);
$template.= '<br />';
...
if (isset($notices) && count($notices)) {
	foreach($notices as $anotice) {
		$rows[] =  array(theme('pmb_external_notice_display', $anotice, 'medium_line', array()));
	}
	...
	$page_path = 'pmb/search/external/'.$search_fields_u.'/'.$search_sources_u.'/'.$search_terms_u.'/';
